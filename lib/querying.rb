...
def select_series_title_with_most_human_characters
  "SELECT series.name FROM series INNER JOIN books ON series.book_id=books.id JOIN character_books ON books.id=character_books.book_id JOIN characters ON characters.id=character_books.character_id GROUP BY character.species ORDER BY COUNT(*) DESC LIMIT 1"
end
